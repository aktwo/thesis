...
The common way of benchmarking the performance of a decision-making algorithm in this context is to take the distance between the expected payout of the algorithm and the optimal payout by play $n$. In the literature, optimality can be defined at two levels of granularity: play-by-play or by expectation. Using the first notion of optimality yields the expected regret, which is defined in equation \eqref{eq:ExpectedRegret}.
...
A slightly weaker notion of regret comes from the second, more broad definition of optimality: optimality in expectation. Using this notion of optimality yields the pseudo-regret, which is defined in equation \eqref{eq:PseudoRegret}.
...
It is important to notice here that in \eqref{eq:ExpectedRegret}, the algorithm is competing with the best possible random draw at every time-step, whereas in \eqref{eq:PsuedoRegret}, the algorithm is only competing with the arm with the highest expected value. This is the intuition for why $\overline{R}_n \leq{\mathbb{E}R_n}$.

Now that the general form of the multi-armed bandit problem has been introduced, there are three main sub-problems outlined by \citet{bubeck12} with further assumptions about the reward distributions. In the stochastic version, the sequences of plays $X_{i, 1}, X_{i, 2}, ... $ are I.I.D samples drawn from distributions $\nu_i \in [0, 1]$ for each arm $i = 1, ..., K$. In the adversarial version, at each time step $t$ an adversary selects a gain vector $g_t = (g_{1, t}, ..., g_{K, t}) \in [0, 1]^{K}$ such that $X_{i, t} = g_{i, t}$. In the Markovian version, the reward processes are neither i.i.d (as in the stochastic version) or chosen by an adversary (as in the adversarial version). Instead, each arm represents a Markov process with a state space $S$. Each time an arm $i$ is chosen in state $s$, a reward is drawn from a probability distribution $\nu_{i, s}$ and the state of the reward process for arm $i$ changes in a Markovian fashion, based on an underlying stochastic transition matrix $M_i$. FIXME REWORD. 

With these three sub-problems in hand, we can turn to the problem at hand: choosing conversation starters for a pair of TA users. Under the simplifying assumption that Princeton students react similarly to different conversation starters, at first glance it makes sense to use the stochastic formulation of the multi-armed bandit problem because the rewards (in this case, Facebook de-anonymization) could reasonably be assumed to be I.I.D. Additionally, the UCB1 algorithm in \citet{auer02} is elegant, computationally easy to implement, and has an upper logarithmic bound on cumulative regret when applied to a stochastic multi-armed bandit problem.

While formulating TA conversation starters as a stochastic multi-armed bandit and using the UCB1 algorithm solves the first problem outlined in Section \ref{WhyMultiArmedBandits} (i.e. picking the conversation starter that students would respond best to), it would actually exacerbate the second problem (i.e. seeing the same conversation starter repeatedly). Since the stochastic multi-armed bandit version of the problem assumes the reward distributions are I.I.D, it will simply hone in on the distribution with the highest expected value, and in the long term, will just pick the optimal arm at every play. In the context of TA, this would result in a single conversation starter always being displayed in the long term. This defeats the purpose of having a variety of conversation starters in the first place.

Since the arms chosen need to depend on the context (i.e. who the users are), a logical first choice would be to look at a contextual bandit algorithm formulation for some help. One example of such a personalized recommendation algorithm can be found in \citet{chu10}. Their algorithm observes the current user $u_t$ and a set $A_t$ of arms of actions together with their feature vectors $x_{t, a}$ for $a \in{A}$. The vector $x_{t, a}$ summarizes information of both the user $u_t$ and arm $a$, and will be referred to as the context. However, such a model would rapidly get out of hand for the case of TA for a variety of reasons. First, there isn't just one user, but rather a pair of users, so using the LinUCB algorithm suggested by \citet{chu10} would FAIL BECAUSE BLAH CONTEXTUAL BANDITS AREN'T A GOOD SOLUTION.
- DEFEATS THE PURPOSE OF THE SIMPLIFYING ASSUMPTION THAT PRINCETON STUDENTS ARE SIMILAR
- 

Another possibility would be to model BANDITS WITH SIDE INFORMATION.

So this raises an important question: how do we take advantage of the simplifying assumption that Princeton students will respond similarly to any given conversation starter (i.e. the rewards are I.I.D) while still enforcing the invariant that a user doesn't see the same conversation starters repeatedly? This was the motivation for me to create a UCB algorithm somewhere between the context-free UCB1 and the context-dependent LinUCB. This new algorithm (which I have named UCB1-AKSB) is weakly context dependent, in that it applies the context-free UCB1 algorithm but dynamically filters the arms over which the algorithm to only include arms that neither user has already seen. This intuitive explanation is formalized in Chapter \ref{Methods} below.
...
Recall from Chapter \ref{LiteratureReview} that the cumulative regret of the UCB1 algorithm is proportional to log($n$), where $n$ is the number of plays. Thus, we should expect the UCB1-AKSB algorithm to result in a cumulative pseudo-regret that is approximately logarithmic in the number of plays. This empirical pseudo-regret, $\hat{R}_n$, was calculated using equation \eqref{eq:RegretComputation} below under the assumptions that the long-term average de-anonymization proportion was optimal.
...
In equation \eqref{eq:RegretComputation}, $\mu^{*}n$ is the expected optimal number of de-anonymizations by play $n$ (i.e. $\max_{i=1,...,K}{\mathbb{E}\left[\sum_{t=1}^{n}{X_{i, t}}\right]}$ from equation \eqref{eq:PseudoRegret}). The plot of $\hat{R}_n$ as a function of $n$ for the UCB1-AKSB algorithm is shown below in \ref{fig:TADe-AnonymizationRegret}.
