...
The first obvious metric by which to judge the UCB1-AKSB algorithm is the proportion of conversations which were de-anonymized. Judging from not only the cumulative de-anonymization proportion (\autoref{fig:TADe-AnonymizationCumulative}), but also the daily de-anonymization proportion (\autoref{fig:TADe-AnonymizationDaily}), it seems that the algorithm had little impact on whether or not people opted to de-anonymize the conversation. However, there are a number of alternative explanations for why this may have been the case: 

- FB De-anonymizations aren't granular enough (it's a binary variable, so it's hard to measure accurately if there isn't much data)
...
- So, if FB de-anonymization was too granular, then let's pick other metrics of conversation quality and see how we're doing there. This makes sense since the original goal of the UCB1-AKSB algorithm was to improve conversation quality (for which conversation de-anonymization served as a proxy). These other metrics (such as participation rates and average conversation length), are convenient to look at because they are more fine-grained than the binary variable of conversation de-anonymization. The plots are below. 
...
FIXME: ADD STUFF ABOUT USER SATURATION
FIXME: REFERENCE APPENDIX WITH NEW CAMPUS ANONYMOUS SITE PLANS?

\section{Individual User Analysis}

Another way of examining the data is to look at how individual users interacted with the site over the course of their site use. In each of the plots below, the x-axis represents the number of uses, and the y-axis represents the conditional mean of the metric over the set of users on the n-th use given that they've used the site greater than or equal to n times. More formally, given a function $f(u, n)$ where $u$ represents the user and $n$ represents the $n$-th play of user $u$ and the function $g(u)$ where BLAH FIXME.
 the plot value represents the conditional mean of the metric 
...
- General upward trend: "power users" are more likely to participate, have high quality conversations, etc.
- Increasing volatility over time represents the stratification of users into different classes. An abrupt change from a participation rate of 100 percent to 0 in one turn is more likely the result of a "power user" being paired with an "amateur" or "first-time" user, so that the first-time user disconnects from the conversation before the "power-user" has a chance to participate at all.

\chapter{Conclusions}

- SUMMARY OF FINDINGS
- General increase in conversation quality, but not in the metric that the UCB1-AKSB was optimizing for
- Although this is disheartening, there is still room for future testing of this algorithm
- IMPROVEMENTS
