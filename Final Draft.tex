...
In this equation $n_x$ is the number of times that conversation starter $x$ has been played and $n$ is the total number of conversation starters that have been shown. Note that ties are broken arbitrarily.
...
The regret looks approximately logarithmic for the second half of the dataset, but the first half of the data gives a steadily negative cumulative regret. This is most likely due to the fact that the distribution of Facebook de-anonymization was not I.I.D. Instead, there were probably different regimes in which people perceived Facebook de-anonymization differently. In the beginning,  people were more likely to de-anonymize a conversation simply because of the novelty of doing so. This is supported by looking at the cumulative Facebook de-anonymization percentage (see \ref{fig:TADe-AnonymizationCumulative}). In the beginning, the Facebook connect rate was almost double the long-term average. However, as users became more used to the idea of Facebook de-anonymization, the rate of de-anonymization drifted slowly back towards the long term average. The existence of multiple regimes explains the two different segments of the regret data. 
...
\begin{itemize}
\item Negative regret occurred because of the initial upsurge in Facebook connects when the site was originally adopted
\item We see the Facebook de-anonymization percentage drift slowly back to the long term mean
\item This regret was calculated by assuming that the long-term average was "optimal" and inferring the "optimal" number of Facebook de-anonymizations that was expected at each number of plays
\item However, this is based on the assumption that the Facebook de-anonymization random variable is being drawn from an I.I.D distribution.
\item I think it's not being drawn from an I.I.D distribution, which is why you see two different paths in the data, (0, 4500) and (4500, end).
\item I think that the bandits algorithm started displaying the expected logarithmic cumulative regret after conversation index (4500)
\item Deanonymization peaked in the beginning (probably the novelty of the site, but then converged to stable levels). This probably was due to the fact that initial de-anonymization would have happened no matter what
\end{itemize}
...
FIXME BLAH
Other proxies for quality (such as participation rates and average conversation length, increased and approached their asymptotic values). It makes sense to use other proxies for quality as a benchmark for cumulative regret of the UCB1-AKSB algorithm, simply by making the assumption that the algorithm optimized for conversation quality.

\begin{figure}[h]
\centering
\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, scale=0.5]{./Pictures/CumulativeTAParticipation.jpg}
\caption{TA Cumulative Participation Data}
